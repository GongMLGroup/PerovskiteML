...
@bind col_sort_name Select(names(data))

# ╔═╡ 6c9291ba-e71a-4c05-a5eb-7b5dadd3cd51
sorted_data = sort(data, col_sort_name)
...
md"""
## Sort By Sparsity
By calculating the total number of missing entries at each column and dividing it by the length of the column the sparsity can be found. This is used to sort the dataset.
"""

# ╔═╡ 35f90a3a-b2b8-4c75-8a32-fe30d5fa418e
begin
	binary_matrix = data_matrix
	column_length, row_length = size(binary_matrix)
	binary_columns = [binary_matrix[:, i] for i in 1:row_length]
	column_counts = sum.(binary_columns)
	column_sparsity = 100 * column_counts ./ column_length
	sparsity_idx = sortperm(column_counts)
	sorted_sparsity = column_sparsity[sparsity_idx]
	sorted_counts = column_length .- column_counts[sparsity_idx]
	sparsity_sorted_data = sorted_data[:, names(sorted_data)[sparsity_idx]]
end
...
sparsity_range_indices = let
	idx1 = findlast(x-> x >= sparsity_range.lower, sorted_counts)
	idx2 = findfirst(x-> x <= sparsity_range.upper, sorted_counts)
	x = idx1 > idx2 ? (idx2:idx1) : (idx1:idx2)
end

# ╔═╡ 37299187-0a36-4cef-a7e4-707aeb25d175
DataFrame(
	"Column Name" => names(data)[sparsity_idx][sparsity_range_indices],
	"Counts" => sorted_counts[sparsity_range_indices]
)
